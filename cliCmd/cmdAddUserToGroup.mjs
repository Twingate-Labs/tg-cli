...
export function getAddUserToGroupCommands(name) {
    ...
    switch (name) {
        case "group":
            cmd = new Command()
                ...
                .action(async (options, groupId, userId) => {
                    ...
                    let res = await client.addUserToGroup(groupId, userId);
                    switch (options.outputFormat) {
                        case OutputFormat.JSON:
                            console.log(JSON.stringify(res));
                            break;
                        default:
                            Log.success(`Add users '${userId}' to group '${groupId}'.`);
                            break;
                    }
                });
            break;
        case "user":
            cmd = new Command()
                .arguments("<groupId:string> [userIds...:string]")
                .option("-o, --output-format <format:format>", "Output format", {default: "text"})
                .description(`Add users to a group`)
                .action(async (options, groupId, userIds) => {
                    const {networkName, apiKey} = await loadNetworkAndApiKey(options.accountName);
                    options.accountName = networkName;
                    let client = new TwingateApiClient(networkName, apiKey, {logger: Log});
