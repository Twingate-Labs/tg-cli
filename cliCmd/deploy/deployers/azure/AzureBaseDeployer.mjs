...
export class AzureBaseDeployer extends BaseDeployer {
    ...
    async checkAvailable(throwError=true) {
        if ( Deno.build.os === "windows" ) {
            // TODO
        }
        else {
            const output = await execCmd(["command", "-v", "az"], {returnOnNonZeroError: true});
            if (typeof output === "string") {
                return true;
            }
            else if (throwError) {
                const errorMsg = "Azure CLI not detected on path. Please check that it is installed.";
                Log.error(errorMsg);
